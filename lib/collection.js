...
Collection.prototype.find = function find(criteria, cb) {
  ...
  //console.log('---COLLECTION::find - '+ this.identity + ' | criteria:');
  ...
  if (whereKeys.length === 1 && whereKeys[0] === 'id' && criteria.where.id) {
    if (criteria.where.id === null || (Array.isArray(criteria.where.id) && !criteria.where.id.length)) {
      return cb(null, []);
    }
...
Collection.prototype.update = function update(criteria, values, cb) {
  ...
  //console.log('---COLLECTION::update - '+this.identity);
  ...
  if (query.criteria.where.ids) {
    this.connection.client.mget({
      index: this.connection.indexName,
      type: this.identity,
      body: {
        ids: query.criteria.where.ids.values
      }
    }, function(err, results) {
      __searchCallback(err, {hits: {hits: results.docs}});
    });
  } else {
    this.connection.client.search({
      index: this.connection.indexName,
  ...
  function __searchCallback(err, results) {
    if(err) return cb(err);
    ...
    async.each(results.hits.hits, function updateRecord(item, next) {
      ...
      if(err) return cb(err);
      if (!updatedRecords.length) return cb(null, []);
...
Collection.prototype.destroy = function destroy(criteria, cb) {
  ...
  //console.log('---COLLECTION::destroy ' + this.identity);
  //console.log(criteria);
  // Ignore `select` from waterline core
  if (typeof criteria === 'object') {
    delete criteria.select;
...
Collection.prototype._parseDefinition = function _parseDefinition(definition) {
  ...
  if (!index.mappings[self.identity]) {
    index.mappings[self.identity] = {};
  }
  if (!index.mappings[self.identity].properties) {
    index.mappings[self.identity].properties = {};
  }

  Object.keys(self.schema).forEach(function(it) {
    if (!index.mappings[self.identity].properties[it]) {
      index.mappings[self.identity].properties[it] = {};
    }
    if (self.schema[it].es) {
      index.mappings[self.identity].properties[it] = self.schema[it].es;
    }

    index.mappings[self.identity].properties[it].type = self.schema[it].es && self.schema[it].es.type ? self.schema[it].es.type : self.schema[it].type;
    if ('json' === index.mappings[self.identity].properties[it].type.toLowerCase()) {
      index.mappings[self.identity].properties[it].type = 'object';
    }
    if ('datetime' === index.mappings[self.identity].properties[it].type.toLowerCase()) {
      index.mappings[self.identity].properties[it].type = 'date';
    }
    if ('string' === index.mappings[self.identity].properties[it].type) {
      index.mappings[self.identity].properties[it].index = self.schema[it].es && self.schema[it].es.index ? self.schema[it].es.index : 'not_analyzed';
    }
  });
...
Collection.prototype._putMapping = function _putMapping(cb) {
  ...
  //console.log(JSON.stringify(this.elasticSearch));
