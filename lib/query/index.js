...
Query.prototype.parseClause = function parseClause(original) {
  ...
  var myparts = _.reduce(original, function parseClausePair(parts, val, key) {
    "use strict";
    if (key === '0') {
      key = 'id';
    }
    ...
    // handle Like Operators for WQL (Waterline Query Language)
      ...
      _.reduce(val, function parseLikeClauses(likes, expression, field) {
        likes[field] = self.parseExpression(field, { like: expression });
        parts.push(likes[field]);
        return likes;
      }, {});
    }
